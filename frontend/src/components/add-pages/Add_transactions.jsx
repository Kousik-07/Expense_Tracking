...
function Add_transactions({ title, buttonText, defaultData, onClose }) {
  ...
  if (defaultData?.transectionTypes) {
    setType(defaultData.transectionTypes);
  ...
  const handelSubmit = async(e) => {
    ...
    try {
      ...
      if (buttonText === "Add") {
        await http.post("/api/transection/create", data)
        mutate("/api/transection/get");
        toast.success("Transection created successfully")
      } else {
        await http.put(
          `/api/transection/update/${defaultData._id}`,
          data
        );
        mutate("/api/transection/get");
        toast.success("Transection updated successfully");
      }
      onClose()
    } catch (error) {
